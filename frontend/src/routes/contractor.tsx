...
function ContractorPortal() {
  ...
  const apply = useMutation({
    ...
    onSuccess: () => { toast.success("Application submitted"); setForm({ company: "", specialty: "", portfolio: "", notes: "" }); qc.invalidateQueries(); },
  ...
  return (
    <div className="container mx-auto px-6 py-16 max-w-5xl">
      ...
      <div className="grid md:grid-cols-2 gap-12 mt-12">
        <form onSubmit={(e) => { e.preventDefault(); apply.mutate(); }} className="space-y-5">
          <h2 className="font-serif text-2xl">Apply / update studio</h2>
          <F label="Company"><Input value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} required /></F>
          <F label="Specialty"><Input value={form.specialty} onChange={(e) => setForm({ ...form, specialty: e.target.value })} placeholder="Residential, hospitality…" /></F>
          <F label="Portfolio URL"><Input value={form.portfolio} onChange={(e) => setForm({ ...form, portfolio: e.target.value })} /></F>
          <F label="Notes"><Textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} /></F>
          <Button type="submit" disabled={apply.isPending}>{apply.isPending ? "Submitting…" : "Submit"}</Button>
        </form>

        <div>
          <h2 className="font-serif text-2xl mb-6">Your booking requests</h2>
          <div className="space-y-4">
            {bookings.data?.length ? bookings.data.map((b: any) => (
              <div key={b._id} className="border border-border p-4">
                <div className="flex justify-between"><strong>{b.type || "Booking"}</strong><span className="text-xs uppercase">{b.status || "pending"}</span></div>
